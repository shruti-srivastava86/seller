...
def make_ajax_form(model, fieldlist, superclass=ModelForm, show_help_text=False, **kwargs):
    ...
    for model_fieldname, channel in fieldlist.items():
        f = make_ajax_field(model, model_fieldname, channel['lookup'], channel['help_text'], show_help_text)
...
def make_ajax_field(related_model, fieldname_on_model, channel, field_help_text, show_help_text=False, **kwargs):
    ...
    """
    from api.generics.ajax_select.fields import AutoCompleteField, \
    ...
    kwargs['field_help_text'] = field_help_text
    if isinstance(field, ManyToManyField):
        kwargs['help_text'] = "* This is a multi-select field"
